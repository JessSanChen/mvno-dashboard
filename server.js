...
function saveTranscriptToFile() {
  const duration = (callEndTime - callStartTime) / 1000;
...
// Function to analyze the complete transcript for spam detection using OpenAI
...
// Analyze the complete transcript every 10 seconds
setInterval(async () => {
  if (fullTranscript.length > 0) {
    const analysisResult = await analyzeForSpam(fullTranscript);
              ...
              if (client.readyState === WebSocket.OPEN) {
                client.send(
                  JSON.stringify({
                    ...
                    text: fullTranscript + (!isFinal ? latestTranscript : ""),
                  })
                );
              }
            });
